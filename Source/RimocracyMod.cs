...
namespace Rimocracy
{
    public class RimocracyMod : Mod
    {
        ...
        public override void DoSettingsWindowContents(Rect inRect)
        {
            ...
            listingStandard.Label($"Min Population for Politics: {Settings.MinPopulation.ToStringCached()}", tooltip: "Most mod effects are disabled if you have fewer citizens than this number");
            Settings.MinPopulation = (int)listingStandard.Slider(Settings.MinPopulation, 1, 20);

            listingStandard.Label($"Citizenship Age: {Settings.CitizenshipAge.ToStringCached()}", tooltip: "Biological age for a colonist to have citizenship rights");
            ...
            listingStandard.Label($"Min Population for Campaigning: {Settings.MinPopulationForCampaigning.ToStringCached()}", tooltip: "Min number of voters to have elections between only two candidates, who actively seek supporters");
            ...
            listingStandard.Label($"Mental State Vote Weight Penalty: {Settings.MentalStateVoteWeightPenalty.ToStringCached()}", tooltip: "How much vote weight is reduced for candidates in a mental state (the value is doubled for aggressive mental states)");
            Settings.MentalStateVoteWeightPenalty = (int)listingStandard.Slider(Settings.MentalStateVoteWeightPenalty, 0, 20);

            listingStandard.Label($"Same Backstory Vote Weight Penalty: {Settings.SameBackstoryVoteWeightBonus.ToStringCached()}", tooltip: "How much vote weight is increased for candidates with similar backstories as the voter");
            Settings.SameBackstoryVoteWeightBonus = (int)listingStandard.Slider(Settings.SameBackstoryVoteWeightBonus, 0, 50);

            listingStandard.Label($"Political Sympathy Factor: {Settings.PoliticalSympathyWeightFactor.ToStringCached()}", tooltip: "The importance of political sympathy for voting");
            Settings.PoliticalSympathyWeightFactor = (int)listingStandard.Slider(Settings.PoliticalSympathyWeightFactor, 0, 50);

            listingStandard.Label($"Random Vote Weight Element Radius: ±{Settings.RandomVoteWeightRadius.ToStringCached()}", tooltip: "Max random number that is added or subtracted from the vote weight");
